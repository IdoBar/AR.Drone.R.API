...
invisible(sapply(list.of.packages, function(p) if (!require(p, character.only=T, quietly=T)) {
...
find_mantissa <- function(f) {
  dec_pos <- ifelse(abs(f)>1,grep(".", unlist(strsplit(as.character(f), split=NULL)), fixed = TRUE), 1)
  rem <- ifelse(abs(f)>1,abs(f*10^(1-dec_pos)), abs(f))
  ...
  return(c(paste0(mantissa[-1], collapse=""),23-i-dec_pos))
}
...
float2Int <- function(decfloat){
  ...
  exp <- dec2binstr(127+trunc(log2(decfloat)))
